...
const renderToDoList = (type) => {
    ...
    todoListArr.forEach((task, index) => {
        if (task.isActive && (type == 'All' || type == 'Active')) {
            todoListHTML += `<li class="draggable" draggable="true"><button name="finish" class="check" onclick="finishTask(${index})"></button>
            ...
            if (!task.isActive && (type == 'All' || type == 'Completed')) {
                todoListHTML += `<li class="draggable complete" draggable="true"><button name="unfinish" class="check on" onclick="unfinishTask(${index})">
